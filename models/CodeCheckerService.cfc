...
component accessors="true" {
	...
	property name="categories" type="string" required="true" getter="true";
	property name="results" type="array" required="true" getter="true";
	property name="rules" type="array" required="true" getter="true";

	this.name = "CodeCheckerService";
	this.setCategories("");
	this.setResults([]);
	this.setRules([]);
	...
	*/
	CodeCheckerService function init( required any rulesService inject="rulesService", required any utilService inject="utilService", string categories = "" ){
		...
		this.setRules(variables.rulesService.getRules());
		return this;
	}

	/**
	* @hint I return whether the code check passed or failed.
	...
	*/
		...
		if ( DirectoryExists(arguments.filepath) ) {
			...
			for ( local.row = 1; local.row LTE local.qryFiles.recordcount; local.row++ ) {
				if ( local.qryFiles.type[local.row] IS "File" ) {
					...
					if ( this.getCategories() IS "_ALL" OR ListFind( this.getCategories(), 'QueryParamScanner') ) {
						runQueryParamScanner(filepath=local.filePath);
					}
					if ( this.getCategories() IS "_ALL" OR ListFind( this.getCategories(), 'VarScoper') ) {
	...
	/**
	* @hint I read the file and run the rules against the file.
	...
	*/
	public void function readFile( required string filepath ) {
		...
		while ( !fileIsEOF( local.dataFile ) ) {
			...
			runRules(filepath=arguments.filepath, line=local.line, linenumber=local.lineNumber, categories=this.getCategories());
	...
	/**
	* @hint I run the code review rules for the line of code.
	* @filepath I am the file path for which to review.
	...
	*/
	public void function runRules( required string filepath, string line, numeric linenumber, string categories = "" ) {
		...
		for ( local.ruleitem in this.getRules() ) {
			...
			if ( arguments.categories IS "_ALL" OR ListFind( arguments.categories, local.ruleitem["category"] ) ) {
				...
				if ( StructKeyExists(arguments,"line") AND NOT local.ruleitem.bulkcheck AND NOT ListLen(local.ruleitem.tagname,"|") ) {
					...
					local.objJREUtils = createObject("component","models.QueryParamScanner.jre-utils").init();
	...
	/**
	* @hint I run the qpscanner component.
	* @filepath I am the file path for which to review.
	*/
	public void function runQueryParamScanner( required string filepath ) {
		...
		if ( ListFindNoCase("cfm,cfc",local.fileextension) ) {
			...
			local.objQueryParamScanner = new models.QueryParamScanner.qpscanner(jre=local.objJREUtils, StartingDir=arguments.filepath, OutputFormat="wddx", RequestTimeout=-1);
	...
	/**
	* @hint I run the varScoper component.
	* @filepath I am the file path for which to review.
	*/
	public void function runVarScoper( required string filepath ) {
		...
		if ( local.fileextension IS "cfc" ) {
			...
			local.objVarScoper = new models.VarScoper.varScoper(fileParseText=FileRead(arguments.filepath));
	...
	/**
	* @hint I record the result of the code review.
	*/
	public void function recordResult() {
		ArrayAppend(this.getResults(), arguments);
	}
